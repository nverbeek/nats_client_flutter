...
- **Row Right-Click Context Menu** (all three message list views — Live Messages tab, JetStream Browse Messages, JetStream Consumer Tail): right-clicking anywhere on a row opens the same action menu as that row's trailing overflow (⋮) button, anchored at the click point via `showMenu`/`RelativeRect` instead of the row's trailing edge. Each view (`lib/main.dart`, `lib/jetstream_message_view.dart`, `lib/jetstream_consumer_tail_view.dart`) has its own `_buildRowMenuItems`/`_handleRowMenuSelection`/`_showRowContextMenu` trio, shared between the overflow button and the right-click `GestureDetector.onSecondaryTapDown` so the two triggers can't drift out of sync within a view.
