...
- **Row Right-Click Context Menu** (all three message list views — Live Messages tab, JetStream Browse Messages, JetStream Consumer Tail — plus KV Store key rows): right-clicking anywhere on a row opens the same action menu as that row's trailing overflow (⋮) button, anchored at the click point via `showMenu`/`RelativeRect` instead of the row's trailing edge. Each view (`lib/main.dart`, `lib/jetstream_message_view.dart`, `lib/jetstream_consumer_tail_view.dart`, `lib/kv_dashboard.dart`) has its own `_buildRowMenuItems`/`_handleRowMenuSelection`/`_showRowContextMenu` trio, shared between the overflow button and the right-click `GestureDetector.onSecondaryTapDown` so the two triggers can't drift out of sync within a view.
...
│   ├── account_info_dialog.dart # Read-only JetStream/KV account usage dialog (memory/storage vs. limits, stream/consumer counts, API stats) shared by both dashboards
│   ├── auth_manager.dart       # `buildAuthConnectOption()`/`isAuthenticationError()` — pure logic for the four non-TLS auth methods (username/password, token, NKey seed, `.creds`)
...
│   ├── service_discovery_manager.dart   # Thin, testable wrapper around client.discoverServices()/getServicesInfo()/getServicesStats()
...
│   ├── subject_chip_style.dart # Shared chip label/background/border style (`SubjectChipStyle`) used by `subject_chips_row.dart`, `subscription_manager_dialog.dart`, and `jetstream_dashboard.dart`'s stream-subject chips, so all three stay visually consistent from one definition
...
- **Widget tests (`test/`)**: no live server. For anything touching `JetStreamManager`, extend the `FakeJetStreamManager` in `test/jetstream_dashboard_test.dart` (override the specific method(s) you need; most already have an overridable `xImpl` function field). For standalone dialogs (`Create Stream`, `Create Consumer`, `Settings`, `Security Settings`, `Message Detail`, `Send Message`), just `pumpWidget` the dialog directly wrapped in a `MaterialApp`/`Scaffold` — see `test/jetstream_stream_dialog_test.dart` or `test/settings_dialog_test.dart` for the pattern. `MyHomePage` itself (Live Messages tab) has no fake-injection point for the *connection* (`natsClient` is constructed internally), so anything requiring a live connection is integration-test-only — but list/selection/shortcut behavior CAN be widget-tested without connecting by pumping `MyHomePage` (wrapped in `ChangeNotifierProvider<ThemeModel>` + `MaterialApp`/`LoaderOverlay`, at a 1280x720 `tester.view` size) and feeding messages straight into `handleIncomingMessage()` via `tester.state(find.byType(MyHomePage)) as dynamic` — see `test/live_messages_shortcut_focus_test.dart` for the pattern. The Browse Messages view is also widget-testable: `OrderedConsumer.messages()`/`stop()` are overridable, see `test/jetstream_message_view_test.dart`'s `_FakeOrderedConsumer`.
