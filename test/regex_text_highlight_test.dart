...
void main() {
  ...
  testWidgets('an invalid regex renders plain text instead of crashing',
      (tester) async {
    await tester.pumpWidget(buildHighlight(text: 'hello [world]', searchTerm: '['));

    expect(tester.takeException(), isNull);
    expect(find.text('hello [world]'), findsOneWidget);
    expect(find.byType(RichText).evaluate().where((e) {
  ...
  testWidgets('a normal pattern highlights the matching portion',
      (tester) async {
    await tester.pumpWidget(buildHighlight(text: 'hello world', searchTerm: 'world'));
