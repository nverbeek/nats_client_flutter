...
- [ ] **Not yet committed** — `lib/connection_history.dart`, `lib/constants.dart`, `lib/main.dart`, and the three test files above are working-tree only as of this writing.
...
- [ ] **Not done in this pass**: the committed `images/*.png` screenshots (captured by `scripts/capture_screenshots.ps1` / `integration_test/screenshot_tour_test.dart`) still show the old `ElevatedButton` styling. Re-capturing them needs the full screenshot-tour automation (Docker + seeded demo buckets/streams) run end-to-end for its own sake, not just the assertions above — left as a follow-up rather than bundled here.
