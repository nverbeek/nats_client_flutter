...
These features are made possible by our successful migration to the official mainline `dart_nats` package (originally `^1.1.1`; now `^1.2.2`, see Milestones 18 and 26).
...
- [x] **Milestone 23**: Adopt Reconnect-Buffer Overflow Handling (`maxReconnectBuffer`) (Low Priority). The three fire-and-forget publish call sites (Send dialog, row-menu Replay, Ctrl+R) now await the publish and catch a full-buffer `NatsException`, surfacing it via a dedicated error-styled SnackBar (`describePublishError()` in `format_utils.dart`) instead of letting it vanish as an unhandled Future rejection.
- [x] **Milestone 24**: Adopt Heartbeat Ping/Pong for Faster Dead-Connection Detection (Low Priority). No app code change was actually needed — `Client.connect()`'s `pingInterval`/`maxPingsOut` defaults (120s / 2 pings) already apply automatically once on `dart_nats: ^1.2.2`, and this app deliberately doesn't override them (no user complaint about reconnect latency to justify tuning). Verified live against a disposable Docker container: `docker pause` (a silent hang with no FIN/RST, unlike `docker kill`) on a client connected with a short custom `pingInterval`/`maxPingsOut` flipped `Status.disconnected` within one interval, confirming the mechanism genuinely detects a dead-but-not-closed connection.
- [ ] **Milestone 25**: Design & Implement **Quick-Subscribe to Exact Subject from Message Row** (Low/Medium Priority). Not started.
- [x] **Milestone 26**: Adopt **dart_nats JetStream/KV Bug Fixes** (Medium Priority) — 4 real bugs (list-pagination truncation, swallowed pull-consumer/OrderedConsumer errors, KV ephemeral-consumer leaks, fire-and-forget ack/nak/term) fixed upstream using this project's new direct contributor access (no fork needed, unlike PR #44) and submitted as `chartchuo/dart-nats` **[PR #45](https://github.com/chartchuo/dart-nats/pull/45)**. Merged and published to pub.dev as **`dart_nats` 1.2.2**; `pubspec.yaml` now depends on `^1.2.2` (no `git:`/`dependency_overrides` pin), verified against the real published release. All follow-up cleanups are done too — see the milestone's own section.
...
## Milestone 23: Adopt Reconnect-Buffer Overflow Handling (`maxReconnectBuffer`) (Low Priority) — Completed
...
### Implementation Checklist
- [x] Wrap the three fire-and-forget publish call sites in `lib/main.dart` with error handling for the new `NatsException` overflow case. `sendMessage()`'s plain-publish branch now awaits `pubString()` in a try/catch; row-menu Replay and Ctrl+R both now go through a new shared `_publishReplay()` helper that awaits and catches (Ctrl+R fires it via `unawaited()` from its synchronous key-event handler, same fire-and-forget-with-internal-error-handling pattern as the existing Export/Replay feature's `_runReplay`).
- [x] Decide and implement the overflow UX: a new `describePublishError()` in `format_utils.dart` names the buffer-full case distinctly ("Too many messages queued while disconnected — this one was not sent.") vs. a generic truncated fallback for anything else, shown via a new shared `_showErrorSnackBar()` helper (the existing `colorScheme.error`/`onError` pattern, also now used by the JetStream publish-error path, replacing its inline duplicate).
- [x] Unit/widget tests for the new catch path: `describePublishError()` is pure logic, unit tested directly in `test/format_utils_test.dart` (buffer-full case, case-insensitivity, generic fallback, non-`NatsException` errors) — driving an actual 1000-message buffer-exhaustion end-to-end through the real UI wasn't pursued (impractical to automate; `natsClient` isn't exposed outside `_MyHomePageState` for a test to seed the buffer directly), so this mirrors the project's existing precedent for other hard-to-drive paths (e.g. the file-picker Browse buttons).
- [x] Live-server verification pass: re-ran `integration_test/live_messages_interactions_test.dart` (exercises Replay/Ctrl+R's happy path through the new `_publishReplay()` helper) against a real disposable `nats:latest -js` Docker container — all 5 cases pass, confirming no regression from the added await/try-catch.

---

## Milestone 24: Adopt Heartbeat Ping/Pong for Faster Dead-Connection Detection (Low Priority) — Completed
...
### Implementation Checklist
- [x] Decide on `pingInterval`/`maxPingsOut` values: kept the package defaults (120s / 2 pings), documented with an inline comment at the `natsClient.connect(...)` call site in `lib/main.dart` explaining the decision.
- [x] Live-server verification: a throwaway `bin/verify_heartbeat.dart` script (not committed) connected a raw `dart_nats` `Client` to a disposable Docker container with a short custom `pingInterval`/`maxPingsOut`, then `docker pause`d the container (freezes the process without closing the socket — a truer simulation of a silently dropped path than `docker kill`, which sends a real FIN/RST the socket layer would notice immediately regardless of any heartbeat). `Status` flipped from `connected` to `disconnected` within the expected interval, confirming the mechanism.
- [x] Confirm no regression on the happy-path reconnect flow: full `flutter test` (308 cases) and the live-server integration suite both pass unchanged, since the app doesn't override the new params.
...
## Milestone 26: Adopt dart_nats JetStream/KV Bug Fixes (Medium Priority) — Completed

### Objective
Four real `dart_nats` bugs were found while building/hardening this app's JetStream/KV features, each worked around app-side rather than fixed at the source, since the app previously had no way to push a fix upstream without maintaining a whole fork:
- `JetStream.listStreams()`/`listConsumers()` silently truncate past 256 items (worked around via `jetstream_manager.dart`'s `listStreamsPaginated`, see its doc comment).
- Pull consumers and `OrderedConsumer` swallow delivery errors forever, retrying silently (worked around via `jetstream_consumer_tail_view.dart`'s 5s health-probe timer).
- `KeyValue.watch()`/`history()`/`keys()` leak an ephemeral consumer on every call (no app-side workaround existed for this one — the leak is server-side, not observable in this app's own behavior).
- `ack()`/`nak()`/`term()`/`inProgress()` are fire-and-forget and can't detect a failed delivery — this is the deferred item **9b** from the earlier bug/perf cluster session: `jetstream_consumer_tail_view.dart`'s Ack/Nak/Term buttons mark rows resolved optimistically with no revert-on-failure, because the package gave no way to know if an ack actually landed.

This project gained direct contributor (write) access to `chartchuo/dart-nats` mid-2026, meaning fixes can now be pushed as a branch directly to the upstream repo and opened as a same-repo PR — no fork-and-cross-repo-PR round trip needed, unlike Milestone 18's PR #44 or the earlier PR #43. All four fixes were made this way: branch `fix/upstream-bug-batch` pushed straight to `chartchuo/dart-nats`, submitted as **[PR #45](https://github.com/chartchuo/dart-nats/pull/45)**, merged by the maintainer 2026-07-17, and published to pub.dev the same day as **`dart_nats` 1.2.2**.

### What Happened
- All 4 fixes were verified against `dart_nats`'s own test suite (166 tests, `dart analyze` + `dart test -j1` against its `docker-compose` NATS servers) and separately re-verified end-to-end against this app (full 304-test suite + live-server integration tests) before the PR was opened, by pointing this app's `pubspec.yaml` at the branch via `dependency_overrides`.
- One of the fixes (the KV consumer leak) needed a second pass mid-session — the regression tests themselves caught a real race in the first attempt (cleanup could run before the consumer's create-response landed, orphaning it anyway) — fixed and reverified before the PR was opened.
- The maintainer flagged "some mixup" with the 1.2.2 release without specifying what. Rather than trust the CHANGELOG/version number alone, the actual published archive was downloaded straight from pub.dev's API (`https://pub.dev/api/archives/dart_nats-1.2.2.tar.gz`) and its `lib/src/` contents grepped directly to confirm the pagination loop, `inactiveThreshold` field, and `nakSync`/`termSync`/`inProgressSync` are all genuinely present in what's actually installable — not just implied by a changelog entry. Whatever the mixup was, the shipped artifact itself is correct.
- **Cut over**: `pubspec.yaml`'s `dependency_overrides` removed, `dependencies: dart_nats` bumped from `^1.2.1` to `^1.2.2`. `pubspec.lock` now resolves to the real hosted release (`source: hosted`, sha256 `9687...5ee782`, matching the pub.dev API's own reported hash for 1.2.2 — no `git:` pin left behind). Full `flutter analyze` + all 304 `test/` cases + `integration_test/kv_lifecycle_test.dart`, `integration_test/jetstream_lifecycle_test.dart`, and `integration_test/jetstream_browse_test.dart` re-run clean against a fresh disposable `nats:latest -js` Docker container.

### Optional Follow-up Cleanup — all done (2026-07-17)
- [x] `lib/jetstream_consumer_tail_view.dart`'s `_ack()`/`_nak()`/`_term()` now switch to `ackSync()`/`nakSync()`/`termSync()` via a shared `_resolve()` helper: optimistic `_resolved.add`, `await` the sync call, and on a thrown `NatsException`/other error, revert (`_resolved.remove`) and show a `colorScheme.error`/`onError`-styled `SnackBar` naming which action failed. This was deferred item **9b** from the bug/perf cluster session — completed 2026-07-17 once 1.2.2 shipped `ackSync`/`nakSync`/`termSync`. Test in `test/jetstream_consumer_tail_view_test.dart` drives a message through a disconnected `Client` (so the sync ack throws immediately, no live server needed) and asserts the button re-enables + the snackbar appears.
- [x] That same file's 5s health-probe timer is now **removed**, not kept as defense-in-depth: PR #45's pull-consumer fix means `Consumer.messages()` genuinely surfaces a deleted/unreachable consumer via the stream's own `onError` now (verified by reading `dart_nats-1.2.2`'s `jetstream.dart` directly — an empty-batch fetch confirms the consumer still exists via `info()`, and any fetch/info failure calls `controller.addError(e)`), so the app-side poll was pure duplication of a bug that's actually fixed. `_startTailing()`'s `onError` handler (already present for the push-consumer case) now also cancels the subscription on first error, matching the old probe's behavior of not continuing to hit a dead consumer. Tests rewritten to drive a fake consumer's stream error directly instead of a fake `consumerInfo()` poll.
- [x] `lib/jetstream_manager.dart`'s `listStreamsPaginated` bypass is **reverted** — `JetStreamManager.listStreams()`, `KvManager.listBuckets()`, and `ObjectStoreManager.listBuckets()` all call `JetStream.listStreams()` directly again, since the official package now paginates internally (this app's own contributed algorithm, merged upstream). The bypass function and its now-unused imports were deleted rather than left as dead code.

All three re-verified against a live disposable `nats:latest -js` Docker container: `jetstream_lifecycle_test.dart`, `jetstream_browse_test.dart`, `kv_lifecycle_test.dart`, and `object_store_lifecycle_test.dart` (exercises the reverted `listStreams()` path) all pass, as does `live_messages_interactions_test.dart` (exercises the new `_publishReplay()` path from Milestone 23). Full `flutter test` (308 cases) and `flutter analyze` clean.
