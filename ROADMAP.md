...
### Optional Follow-up Cleanup (evaluate each independently)
- [x] `lib/jetstream_consumer_tail_view.dart`'s `_ack()`/`_nak()`/`_term()` now switch to `ackSync()`/`nakSync()`/`termSync()` via a shared `_resolve()` helper: optimistic `_resolved.add`, `await` the sync call, and on a thrown `NatsException`/other error, revert (`_resolved.remove`) and show a `colorScheme.error`/`onError`-styled `SnackBar` naming which action failed. This was deferred item **9b** from the bug/perf cluster session — completed 2026-07-17 once 1.2.2 shipped `ackSync`/`nakSync`/`termSync`. New test in `test/jetstream_consumer_tail_view_test.dart` drives a message through a disconnected `Client` (so the sync ack throws immediately, no live server needed) and asserts the button re-enables + the snackbar appears.
- [ ] That same file's 5s health-probe timer (workaround for the swallowed-pull-consumer-error bug) could be replaced by listening to the pull consumer's now-real stream errors directly, or kept as defense-in-depth alongside them — a deliberate design decision, not an automatic removal.
- [ ] `lib/jetstream_manager.dart`'s `listStreamsPaginated` bypass can revert to calling `JetStream.listStreams()` directly, or stay as-is — harmless either way now that both implementations do the same thing.
