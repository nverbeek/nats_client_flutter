...
### Optional Follow-up Cleanup (not done, not required — evaluate each independently)
- [ ] `lib/jetstream_consumer_tail_view.dart`'s `_ack()`/`_nak()`/`_term()` can switch to the new `ackSync()`/`nakSync()`/`termSync()` with proper `await` + try/catch + optimistic-revert-on-failure UI. This is deferred item **9b**, finally unblocked.
